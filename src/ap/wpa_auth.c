...
static const struct security_profile_entry_ap
...
/**
 * wpa_auth_sp_implied_key_mgmt - Derive implied key_mgmt from security profiles
...
int wpa_auth_sp_implied_key_mgmt(const int *profiles)
{
	...
	for (i = 0; profiles[i] >= 0; i++) {
		const struct security_profile_entry_ap *sp;

		sp = wpa_auth_sp_get(profiles[i]);
		if (sp)
			implied |= sp->key_mgmt;
	}
